...
import com.info121.ifeedback.models.UpdateProfileReq;
...
public class UserProfileActivity extends AbstractActivity {
    ...
    @BindView(R.id.old_profile_code)
    TextView mOldProfileCode;

    @BindView(R.id.new_profile_code)
    EditText mNewProfileCode;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        APIClient.GetUserProfile(App.Profile_Code);

        profile = new UpdateProfileReq(UUID.randomUUID().toString(), Utils.getDeviceID(mContext));
    ...
    private void populateUserDetail(UserProfileRes userDetail) {
        mFullName.setText(userDetail.getUsername());
      ...
      ///  mProfileImage.setImageBitmap();
        ...
        for (int i = 0; i < sourceAdapter.getCount(); i++) {
            if (sourceAdapter.getItem(i).getSourcecode().equals(userDetail.getSourcecode())) {
                mSource.setSelection(i);
                break;
            }
        }

        mAddress.setText(userDetail.getAddress());
        mOldProfileCode.setText("Profile Code : " +  userDetail.getProfilecode());
    ...
    private void updateProfile(UpdateProfileReq p) {
        pd = ProgressDialog.show(UserProfileActivity.this, "", "Sending...", true, false);
        ...
        APIClient.updateUserProfile(
                ...
                getTextRequestBody(p.getProfileCode())
        );
    }
    ...
    public void btnSaveOnclick() {
        if (validateProfile()) {
            ...
            profile.setProfileCode(userDetail.getProfilecode());
